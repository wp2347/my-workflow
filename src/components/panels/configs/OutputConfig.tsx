...
export function OutputConfig({ node }: OutputConfigProps) {
  ...
  return (
    <div className="space-y-4">
      {runResult && (
        <>
          {runResult.kind === "file" || runResult.filePath ? (
            <div className="space-y-2">
              ...
              <FileResultCard
                ...
                fileSize={runResult.fileSize}
                preview={runResult.preview}
                onCleared={() => clearNodeResult(workflowId!, node.id)}
              />
            </div>
          ) : (
            <div className="space-y-2">
              ...
              <div className="rounded-xl border border-node-music-bg bg-node-music-bg/40 p-3">
                {runResult.audioUrl && <MusicPlayer audioUrl={runResult.audioUrl} fileName={runResult.fileName} />}
              </div>
